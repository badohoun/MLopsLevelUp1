...
function test:quick {
    ...
    --cov "$THIS_DIR/src/customersatisfaction" \
    --cov-report html \
    --cov-report term \
    --cov-report xml \
...
function test:ci {
    # save the exit status of tests 
    PYTEST_EXIT_STATUS=0
    INSTALLED_PKG_DIR="$(python -c 'import customersatisfaction; print(customersatisfaction.__path__[0])')"
    python -m pytest -m 'not slow' "$THIS_DIR/tests/" \
...
function test:all {
    ...
    if [ $# -eq 0 ]; then  
       python -m pytest  "$THIS_DIR/tests/" \
        --cov="$THIS_DIR/customersatisfaction" \
        --cov-report html 
    else 
       python -m pytest "$@"
    fi 
